...
| Unsigned package staging | `build/app/build.py` | `just app-package` | ✅ |
| Workspace boundary CI | Bazel | `bazel test //app:app_ci` | ✅ |
| 平台签名和验证 | `build/app/signing.py` | 包内 target 选择对应签名工具 | ✅ 平台不能由调用者另行指定 |
...
- Windows 使用 `signtool` 和 `ASH_WINDOWS_SIGNING_THUMBPRINT`，签名时加入 RFC 3161
  ...
  `build/app/signing.py record`，不把代码签名私钥下载到 runner；
...
由 Azure Artifact Signing 修改 binary 后运行 `build/app/signing.py record`，重新验证签名并更新
