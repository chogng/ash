...
`cli/` 是独立命令入口。`app-ts/` 和 `app-rs/` 直接连接 App Server，不调用此程序。
...
`scripts/code/` whenever they change on `main`. Its static artifact contains `.nojekyll`
...
`cli/tests/remote_connect.rs` exercises target resolution, the real local Remote Server
broker, trusted runtime preparation, and `--check` through a fake OpenSSH executable.
`cli/tests/remote_connect_interactive.rs` runs the real CLI/TUI in a PTY, cuts the first
