...
test("product build and launch tools have one owner while repository scripts remain shared", () => {
  for (const directory of ["app-ts/scripts", "scripts/app-ts", "scripts/app_ts", "scripts/code", "build/remote", "cli"]) {
    const path = join(repositoryRoot, directory);
    assert.equal(existsSync(path), false, `${directory} must not own product tooling`);
  }
  assert.equal(existsSync(join(repositoryRoot, "ash-cli/Cargo.toml")), true);
  for (const category of ["app_ts", "app_rs", "code", "ash_rs", "download", "lib", "pnpm", "darwin", "win32", "linux", "resources"]) {
  ...
  for (const entry of ["electron.ts", "web.ts", "electron.test.ts"]) {
    assert.equal(existsSync(join(import.meta.dirname, "app_ts/launch", entry)), true, entry);
  ...
  assert.equal(existsSync(join(import.meta.dirname, "app_rs/remote/bundle.py")), true);
