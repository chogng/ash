...
test("Vite hot reload injects setup and a generic export-handler boundary", () => {
  const setupPath = resolve("/workspace/build/app_ts/vite/setup-dev.ts");
  const plugin = hotReloadPlugin({ desktopRoot: "/workspace/app-ts", setupPath });
  const transformed = transform(plugin, "export class SidebarPart extends PaneCompositePart {}", "/workspace/app-ts/src/ash/sidebarPart.ts?direct");
...
test("Vite hot reload emits a valid Windows file URL for setup", () => {
  const plugin = hotReloadPlugin({ desktopRoot: "C:\\workspace\\app-ts", setupPath: "C:\\workspace\\build\\app_ts\\vite\\setup-dev.ts" });
  const htmlTags = plugin.transformIndexHtml.handler();

  assert.deepEqual(htmlTags, [{ tag: "script", attrs: { type: "module", src: "/@fs/C:/workspace/build/app_ts/vite/setup-dev.ts" }, injectTo: "head-prepend" }]);
});

test("Vite hot reload supports explicit prototype-patch opt in", () => {
  const plugin = hotReloadPlugin({ desktopRoot: "/workspace/app-ts" });
  const transformed = transform(plugin, "// @ash-hot-reload patch-prototype\nexport class CustomSurface extends BaseSurface {}", "/workspace/app-ts/src/ash/customSurface.ts");
  assert.ok(transformed);
...
test("Vite hot reload exposes general runtime exports for helper-driven invalidation", () => {
  const plugin = hotReloadPlugin({ desktopRoot: "/workspace/app-ts" });
  const transformed = transform(plugin, "export function compute() { return 1; } export const label = 'one';", "/workspace/app-ts/src/ash/feature.ts");
  assert.ok(transformed);
...
test("Vite hot reload leaves type-only modules and production builds unchanged", () => {
  ...
  assert.equal(transform(plugin, "export interface Configuration { value: string; }", "/workspace/app-ts/src/ash/configuration.ts"), undefined);
  assert.equal(transform(plugin, "interface Configuration { value: string; } export type { Configuration };", "/workspace/app-ts/src/ash/configuration.ts"), undefined);
  assert.equal(transform(plugin, "export declare const injected: string;", "/workspace/app-ts/src/ash/environment.ts"), undefined);
  assert.equal(transform(plugin, "export class SidebarPart extends PaneCompositePart {}", "/workspace/app-ts/src/ash/sidebarPart.js"), undefined);
});
...
test("Vite hot reload sends a full reload before an unsafe module executes", async () => {
  const plugin = hotReloadPlugin({ desktopRoot: "/workspace/app-ts" });
  const file = "/workspace/app-ts/src/ash/sidebarPart.ts";
...
test("Vite hot reload lets helper-driven modules reach the runtime handler", async () => {
  const plugin = hotReloadPlugin({ desktopRoot: "/workspace/app-ts" });
  const file = "/workspace/app-ts/src/ash/feature.ts";
