...
if (args.length === 1 && args[0] === '--prepare') {
  ...
  if (targets.includes('renderer')) {
    ...
    const child = spawn(process.execPath, [resolve(root, 'build/node_modules/vite/bin/vite.js'), 'build', '--config', resolve(root, 'build/app_ts/vite/vite.config.ts')], {
      cwd: resolve(root, 'app-ts'), stdio: 'inherit', windowsHide: true,
    });
