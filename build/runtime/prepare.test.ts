...
test("selects the latest published package and rejects invalid manifests", async () => {
  ...
  try {
    const store = join(root, ".build/runtime/dev/store-v1/x86_64-pc-windows-msvc/packaged-node/dev-small");
...
test("assembles and validates the canonical Windows development layout", async () => {
  ...
  try {
    ...
    assert.deepEqual(extensionPackages, ["css", "go", "html", "javascript", "json", "markdown-basics", "python", "rust", "shellscript", "sql", "theme-defaults", "theme-seti", "toml", "typescript-basics", "xml", "yaml"]);
