...
class PackageTests(unittest.TestCase):
    ...
    def test_code_package_composes_a_canonical_runtime(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            self.assertTrue(
                {"voiceHost", "collaborationServer", "livekit"}
                <= set(system_signing_artifacts(output, spec))
            )
            ...
            with self.assertRaisesRegex(RuntimeError, "update public key"):
                build_code_package(
                    runtime_output, root / "missing-update-key", cli_binary, "bad"
                )
