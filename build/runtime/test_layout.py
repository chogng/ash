...
from build.runtime.ripgrep import RipgrepResolution
...
class LayoutTests(unittest.TestCase):
    def test_development_and_release_produce_the_same_package(self):
        script = f"""
import {{ assemblePackage }} from {json.dumps((ROOT / "build/runtime/layout.ts").as_uri())};
let text = '';
        ...
        for target, platform in [
            ...
            for packaged_node in (False, True):
                with (
                    ...
                    for name in (
                        "build/runtime/layout.ts",
                        "build/runtime/layout.json",
                        "build/runtime/productServices.ts",
                    ...
                    self.assertFalse(
                        (
                            isolated
                            / "app-ts/src/ash/platform/app-server/common/generated"
                        ).exists()
                    )
                    with patch(
                        "build.runtime.layout.__file__",
                        str(isolated / "build/runtime/layout.py"),
                    ):
