...
function packageUsage(): Error {
  return new Error("Usage: node build/runtime/prepare.ts [--javascript-runtime host-provided-node|packaged-node] [--remote-runtime-bundle <bundle-directory>] [--remote-runtime-catalog-url <https-catalog.json> --remote-runtime-catalog-sha256 <digest>]");
}
...
export async function prepareDevelopmentPackage(
  ...
  const outputDirectory = runtimeBuildPath(repositoryRoot, "dev", "store-v1", target, javascriptRuntime, developmentBuildProfile);
  ...
  const layout = JSON.parse(await readFile(join(repositoryRoot, "build/runtime/layout.json"), "utf8")) as {
    readonly licenses: readonly { readonly source: string }[];
  };
  const packageSources = (await readdir(join(repositoryRoot, "build/runtime")))
    .filter(name => name === "layout.json" || name.endsWith(".ts") && !name.endsWith(".test.ts"))
    .map(name => join(repositoryRoot, "build/runtime", name));
  const inputPaths = [
    ...packageSources,
