...
def _sandbox(context):
    ...
    if (
        ...
        or entry.get("path") != "bin/ash-windows-sandbox.exe"
    ):
    ...
    if (
        ...
        or artifact.resolve().parent != context["artifact"].parent
    ):
...
def sign_package(
    ...
    if sandbox is not None and sha256(sandbox[0]["artifact"]) != sandbox[1]["sha256"]:
        raise RuntimeError(
            "staged Windows sandbox digest does not match app-package.json"
        )
    ...
    if sandbox is not None:
        ...
        run_command(
            sign_command(helper_context, config, platform, identity_for(config)), runner
        )
        helper["sha256"] = sha256(helper_context["artifact"])
        record["windowsSandbox"] = {
            "unsignedSha256": helper_unsigned,
            "signedSha256": helper["sha256"],
        }
...
def verify_package(
    ...
    if sandbox is not None:
        ...
        if (
            ...
            or helper_record.get("signedSha256") != helper["sha256"]
            or sha256(helper_context["artifact"]) != helper["sha256"]
        ):
            raise RuntimeError(
                "Windows sandbox signature record digest does not match the executable"
            )
