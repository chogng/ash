...
| `@` | 本地文件、已生效 Plugin 与 OnDemand Instruction；普通文件和 Plugin 作为文字提交，指令选择另附固定摘要引用 |
...
文件补全只识别空白分隔的 `@token`，不处理邮箱中的 `@`。普通文件扫描遵守 Git 忽略规则、不跟随符号链接，并跳过 `.git`、`.ash`、`node_modules` 和 `target`；结果按匹配分数与路径稳定排序，最多 50 项。OnDemand Instruction 从 App Server 的授权目录列表进入同一补全弹层，选中后以 `InstructionRef` 提交，输入普通 `@file.md` 不会把文件提升为指令。文件搜索请求同时校验查询文本和版本，关闭补全后释放搜索句柄。
