...
| ChatGPT 账号额度、点数与重置时间（`/usage`） | [usage.rs](src/usage.rs)，每次打开通过 App Server 查询；Esc 关闭，再次运行刷新 |
...
- `/memories` 按个人、项目、目录展示记忆；Tab / Shift+Tab 切页签并聚焦页签栏，方向键进入搜索与列表。
- `/` 搜索当前范围的全部记忆，Enter 提交；加载更多保留搜索条件。n 新增，a 打开操作菜单，s 选择具体范围，p 调整范围授权，c 前往 Config。
