...
#[test]
fn fork_creates_an_independent_session_without_switching_or_starting_without_prompt() {
    ...
    execute(
        &mut conversation,
        &mut client,
        invocation(TuiSlashCommandAction::Fork, ""),
        &mut app,
    );
    assert_eq!(conversation.session_id(), &original_session);
    assert_eq!(conversation.thread_id(), &original_thread);
    let sessions = client.list_sessions().unwrap().sessions;
    assert_eq!(sessions.len(), 2);
    let copied = sessions
        .iter()
        .find(|session| session.session_id != original_session)
        .unwrap();
    assert_eq!(copied.threads.len(), 1);
    assert_eq!(
        copied.threads[0].forked_from_id.as_ref(),
    ...
    let text = format!("{}\n{}", message.text(), message.detail().unwrap())
        .replace(copied.session_id.as_str(), "SESSION");
    insta::assert_snapshot!("fork_waiting_notice", text);
    execute(
        &mut conversation,
        &mut client,
        invocation(TuiSlashCommandAction::Resume, copied.session_id.as_str()),
        &mut app,
    );
    assert_eq!(conversation.session_id(), &copied.session_id);
    drop(client);
    fs::remove_dir_all(state_root).unwrap();
}

#[test]
fn fork_prompt_starts_a_turn_only_in_the_copied_session() {
    let (mut client, state_root, _) = client_with_model_probe();
    let mut conversation = ActiveConversation::start(&mut client, "original".into()).unwrap();
    let original_session = conversation.session_id().clone();
    let original_thread = conversation.thread_id().clone();
    let mut app = App::new();
    execute(
        &mut conversation,
        &mut client,
        invocation(TuiSlashCommandAction::Fork, "investigate independently"),
        &mut app,
    );
    assert_eq!(conversation.session_id(), &original_session);
    assert_eq!(conversation.thread_id(), &original_thread);
    ...
    let thread = client
        .read_session_thread(SessionThreadReadParams {
            session_id: copied.session_id.clone(),
            thread_id: copied.threads[0].thread_id.clone(),
            history: None,
        })
        .unwrap()
        .thread;
    assert_eq!(thread.turns.len(), 1);
    assert!(thread.turns[0].items.iter().any(|item| matches!(item, ash_protocol::ThreadItem::UserMessage { text, .. } if text == "investigate independently")));
    let original = client
        .read_session_thread(SessionThreadReadParams {
    ...
    let text = format!("{}\n{}", message.text(), message.detail().unwrap())
        .replace(copied.session_id.as_str(), "SESSION");
    insta::assert_snapshot!("fork_background_notice", text);
    drop(client);
    fs::remove_dir_all(state_root).unwrap();
}
