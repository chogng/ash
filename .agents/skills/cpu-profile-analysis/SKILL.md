...
- Analyzing Ash user timing marks like `code/didResolveTextFileEditorModel` (trace files)
- Understanding multi-process behavior (Browser, Renderer, GPU processes in trace files)

## Detecting File Type

- **`.cpuprofile`**: Top-level JSON with `nodes`, `samples`, `timeDeltas` keys. Created by the Ash profiler.
...
DevTools traces are the future of perf tracing for Ash. They are created from the built-in Electron/Chrome DevTools Performance tab and contain far more information than `.cpuprofile` files.
...
| `blink.user_timing` | Ash performance marks (e.g. `code/willResolveTextFileEditorModel`, `code/didResolveTextFileEditorModel`) |
...
|---------|------|------------|
| **Renderer** (pid varies) | Ash's renderer process -- where JS runs | `CrRendererMain` (main thread) |
...
For Ash perf analysis, focus on the **Renderer process, CrRendererMain thread** -- this is where JavaScript execution, layout, and painting happen.
...
Ash emits `performance.mark()` calls that appear as `blink.user_timing` events. These are the most direct way to measure Ash-specific milestones:
...
- **User timing marks**: Ash milestone events and their timestamps
...
- Use `args.data.url` in `FunctionCall` and `EvaluateScript` events to map back to Ash source files (paths like `ash-file://ash-app/Users/.../out/ash/...`).
