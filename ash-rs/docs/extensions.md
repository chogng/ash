...
- 通知只送达成员当前活动 Turn，不启动空闲 Agent，也不转存到后续 Turn。每条最多预览 150 字符，每个 Turn 保留最近 64 条；超出时说明丢弃的旧预览数量，模型上下文有限时优先保留新预览，并提示通过 `board_read` 查看完整讨论。通知失败不撤回已保存的消息，操作重试不会再次发出通知。通知作为带来源的 Agent 消息提供，不与目录规则或其他指令合并。同一运行实例内，发帖投递与订阅修改按讨论板串行化，退订完成后不会再收到尚未投递的旧帖子通知。
