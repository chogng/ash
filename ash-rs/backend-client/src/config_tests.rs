...
fn config_rejects_incomplete_fragments_and_layers() {
    ...
    for document in [
        ...
        for field in ["config_toml", "requirements_toml"] {
            let client = Client::response(200, &json!({field:document}).to_string());
            assert_eq!(
...
fn settings_preserve_explicit_false_and_reject_non_boolean_values() {
    ...
    let client = Client::response(200, r#"{"commit_attribution_enabled":"false"}"#);
    assert_eq!(
        BackendClient::new(&client, &target, RouteStyle::ChatGpt)
            .unwrap()
            .read_user_settings(&CancellationSource::new().token()),
...
fn workspace_messages_keep_order_unknown_types_and_archive_timestamps() {
    ...
    let target = target(CHATGPT_BACKEND_BASE_URL);
    let messages = BackendClient::new(&client, &target, RouteStyle::ChatGpt)
        .unwrap()
