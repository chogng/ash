...
fn completed_task_fixture_extracts_only_user_and_assistant_text_and_the_pr_diff() {
    let client = Transport::response(
        200,
        include_str!("../../tests/fixtures/task_completed.json"),
    );
    let target = target(BASE_URL);
    let details = Client::new(&client, &target, RouteStyle::ChatGpt)
        .unwrap()
...
fn failed_task_fixture_preserves_error_without_inventing_content() {
    let client = Transport::response(200, include_str!("../../tests/fixtures/task_failed.json"));
    let target = target(BASE_URL);
    let details = Client::new(&client, &target, RouteStyle::ChatGpt)
        .unwrap()
...
fn task_creation_preserves_payload_and_requires_an_unambiguous_task_id() {
    ...
    for body in [r#"{"task":{"id":"new"}}"#, r#"{"id":"new"}"#] {
        let client = Transport::response(201, body);
        let backend = Client::new(&client, &target, RouteStyle::ChatGpt).unwrap();
        assert_eq!(
            backend
