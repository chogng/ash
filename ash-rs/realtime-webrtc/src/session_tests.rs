...
fn controls_preserve_order_and_independent_settings_and_overflow_closes() {
    ...
    handle.set_microphone(MicrophoneState::Muted).unwrap();
    handle.set_speaker(SpeakerState::Suppressed).unwrap();
    handle.set_microphone(MicrophoneState::Active).unwrap();
    ...
    assert!(handle.set_microphone(MicrophoneState::Active).is_err());
...
fn controls_queued_during_negotiation_precede_first_device_enable() {
    ...
    let (complete, _) = blocking::sync_channel(1);
    handle
        .send(Command::Answer(
...
#[test]
fn package_availability_requires_helper_and_platform_runtime() {
    let unique = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("voice-package-{}-{unique}", std::process::id()));
    let voice = root.join("ash-resources/voice");
