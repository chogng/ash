...
/// A cloned handle shares ownership; platform media remains entirely inside the child process.
#[derive(Clone)]
...
/// Whether a session may send microphone audio.
...
/// Whether a session may render received audio.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
impl RealtimeWebrtcSession {
    /// Check package availability without loading platform code or touching any audio device.
    ...
    pub fn is_supported() -> bool {
        cfg!(any(
            ...
            .package_layout()
            .is_some_and(|package| package_has_runtime(package.package_directory()))
    }
    ...
    pub fn start(abort: AbortRegistration) -> Result<StartedRealtimeWebrtcSession> {
        let package = install_context::InstallContext::current()
            .package_layout()
            .cloned()
            .ok_or_else(|| anyhow::anyhow!("voice package unavailable"))?;
        let build_commit = build_info::BuildInfo::current()
            .commit
            .ok_or_else(|| anyhow::anyhow!("voice requires a stamped build"))?;
        let (sender, receiver) = mpsc::channel(8);
        let (offer, result) = blocking::sync_channel(1);
...
fn package_has_runtime(package: &std::path::Path) -> bool {
    let voice = package.join("ash-resources/voice");
    let (helper, runtime) = if cfg!(target_os = "macos") {
        ("bin/ash-voice-host", "lib/libgstreamer-1.0.0.dylib")
    } else if cfg!(windows) {
        ("bin/ash-voice-host.exe", "bin/gstreamer-1.0-0.dll")
    } else {
        ("bin/ash-voice-host", "lib/libgstreamer-1.0.so.0")
    };
...
impl RealtimeWebrtcSessionHandle {
    ...
    pub fn set_microphone(&self, state: MicrophoneState) -> Result<()> {
        let mut controls = self
            .0
            .controls
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        controls.microphone_muted = state == MicrophoneState::Muted;
        self.send(Command::Controls(*controls))
    }

    pub fn set_speaker(&self, state: SpeakerState) -> Result<()> {
        let mut controls = self
            .0
            .controls
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        controls.speaker_suppressed = state == SpeakerState::Suppressed;
        self.send(Command::Controls(*controls))
    }
    ...
    pub fn apply_answer_sdp(
        ...
        let (complete, result) = blocking::sync_channel(1);
    ...
    pub fn take_microphone_peak(&self) -> u16 {
        self.0.state.microphone.swap(0, Ordering::AcqRel)
    }

    pub fn take_speaker_peak(&self) -> u16 {
        self.0.state.speaker.swap(0, Ordering::AcqRel)
    }
...
// Keep diagnostics bounded and independent of untyped platform, SDP, or device error text.
