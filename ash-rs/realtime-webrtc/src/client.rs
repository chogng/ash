...
use anyhow::Result;
use anyhow::ensure;
use install_context::PackageLayout;
...
const RUNTIME_INITIALIZATION_DEADLINE: Duration = Duration::from_secs(30);
...
impl VoiceHost {
    ...
    pub async fn start_transport(mut self) -> Result<(Self, crate::SessionDescription)> {
        let response = self
            .request(Message::StartTransport {}, Duration::from_secs(20))
            .await?;
    ...
    pub async fn apply_answer(mut self, sdp: crate::SessionDescription) -> Result<Self> {
        let response = self
            .request(Message::ApplyAnswer { sdp }, Duration::from_secs(20))
            .await?;
    ...
    /// Initialize the packaged platform runtime without opening devices or starting a session.
    ...
    pub async fn connect(package: &PackageLayout, build_commit: &str) -> Result<Self> {
        let root = package.package_directory().canonicalize()?;
        let name = if cfg!(windows) {
            "ash-voice-host.exe"
        } else {
            "ash-voice-host"
        };
        let path = root.join("ash-resources/voice/bin").join(name);
        ensure!(
        ...
        let SpawnedProcess {
            ...
            stderr_rx,
            exit_rx,
        } = pty::spawn_pipe_process(
            path.to_str()
                .ok_or_else(|| anyhow::anyhow!("voice helper path is not UTF-8"))?,
            &[],
            &root,
            &environment,
            &None,
        ...
        host.exchange(
            ...
            Message::Ready {},
            // Startup-linked platform libraries load before the helper can acknowledge Hello.
            RUNTIME_INITIALIZATION_DEADLINE,
        )
        .await?;
        Ok(host)
    ...
    async fn next_response(&mut self) -> Result<Message> {
        match self.output.next().await {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::UnexpectedEof => {
                ...
                // status; never log its untyped stderr, SDP, or platform error text.
                let exit_code = match self.observed_exit {
                    Some(status) => status,
                    None => match timeout(Duration::from_millis(250), &mut self.exit).await {
                        Ok(status) => {
