...
Ash Agent `grep` 的 tgrep 进程与协议适配器。App Server 按 Directory 持有一个 `Session`；索引、压缩和文件监听由包内 tgrep 负责。

## 安装与生命周期

- 版本固定为 1.0.8，平台 archive 与 SHA-256 见 [`runtime-lock.json`](../../third_party/tgrep/runtime-lock.json)。升级需同时更新适配器版本与 App Server 索引子目录。
...
索引查询先取匹配文件，再分批获取内容，以实现按路径顺序的全局 100 行上限。单文件、正向 glob 覆盖或初始索引未就绪时调用同一 executable 的 `--no-index`，遵循 tgrep 的扫描规则。每行在 App Server 最多展示 500 字符。忽略大小写通过正则内联 `(?i)` 传递，规避 1.0.8 独立大小写标志的 Unicode trigram 漏选。
...
旧 `fastRegex` 配置由 Config 迁移为 `tgrep`。旧 FRS 索引不读取；关闭并删除由 App Server 经 State Runtime 租约删除整个 Agent grep 索引目录。

## 验证

```sh
just test ash-tgrep
just test ash-app-server --lib agent_grep
just test ash-app-server --lib tgrep_rpc
