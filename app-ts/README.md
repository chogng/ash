...
与 Preload 仍会重启整个 Electron 进程。`scripts/electron.ts --watch` 调用 `build/app_ts/compilation.ts`
...
完整 Electron 开发命令还会运行 `build/app_ts/watch.ts app-server`。Rust 源码或 Cargo manifest 变化后，它先完成
