...
| Font cache persistence | Current | `workbench/browser/workbench.ts` restores and warms fonts before creating editor surfaces, then saves `editorFontInfo` in application storage with a machine target on state saves and direct disposal. Restored metrics are validated and remain untrusted; serialization returns `undefined` until a current-session measurement occurs, preserving earlier storage even after expiry. Completed shutdown is not saved again during disposal |
