...
- 修改 line identity、store validation、transaction、selection、history、plugin 或 serialization：运行 `pnpm --dir app-ts run test:editor:unit`。
- 修改 `RichTextEditorWidget`、clipboard、IME、atom view 或 pane integration：运行 unit suite 和 `pnpm --dir app-ts run test:editor:browser`。
