...
| Open Agents Window | `contrib/openAgentsWindow/browser/` and `contrib/openAgentsWindow/electron-browser/` | own the browser titlebar command and desktop command; shared selection and synchronization live in `workbench/contrib/keybindings/`, while `platform/globalKeybindings/` owns operating-system registrations |
