...
顶部的分支选择器有两个创建入口：“新建工作树”（`w`）直接创建 detached 的独立受管 worktree 和新 Session；“新建分支”（`b`）先输入预填 `ash/` 的分支名，再为新 Session 创建该分支及独立受管 worktree。两者都由 App Server 为新根 Thread 绑定执行目录，来源项目目录及其当前分支保持不变。顶部的分支和目录显示来源项目目录；选择器中的现有分支仍走该目录的 Git 分支切换。
