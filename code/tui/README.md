...
首页“新建任务”提供两种工作树：默认 detached 的独立受管 worktree（`w`），或输入预填 `ash/` 的新分支名后创建分支工作树（`b`）。两者都由 App Server 为新根 Thread 绑定执行目录，来源项目目录及其当前分支保持不变。直接在首页输入任务仍走默认 detached 工作树。顶部的“项目分支”面板只管理来源项目目录的本地分支：可切换未被占用的分支，也可用 `b` 在 HEAD 新建分支而不切换目录或开始任务；已在其他工作树检出的分支会标明占用。
