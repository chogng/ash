...
`LanguageService` 只管理语言 ID 与文件关联，`ComposableLanguageConfigurationService` 只管理括号、注释、缩进等编辑规则，`LanguageFeaturesService` 只管理能力 provider registry。Standalone 的 `languages` API 和 Workbench 的 App Server、TextMate、扩展适配器都写入这组共享 registry；具体 Hover、补全、折叠等 contribution 消费 registry 并拥有自身请求与展示流程，不要求额外创建 model-level service。公共 provider 契约归 `common/languages.ts`，Editor 不读取服务器 DTO。格式化契约已迁入该公共文件；文档格式化使用 `DocumentFormattingEditProvider`，从 `ITextModel` 读取资源和语言，通过 `CancellationToken` 接收取消。范围格式化使用 `DocumentRangeFormattingEditProvider`，范围作为独立参数传入。`editor.action.formatSelection`（Ctrl/Cmd+K、Ctrl/Cmd+F）调用当前最高优先级范围提供者；空选区扩展为当前行，多选区优先使用批量接口，否则逐范围查询并一次提交。选区变化取消等待，所有编辑共享一次撤销；命令可用状态随提供者注册、注销和模型语言变化更新。逐范围结果发生交叠时，合并对应请求范围重新查询，并丢弃旧结果；提供者冲突选择仍未对齐；输入格式化使用 `OnTypeFormattingEditProvider`，要求提供者声明触发字符；Standalone 支持显式注册，但 `formatOnType` 自动触发链仍未接通。扩展注册协议尚未提供触发字符，因此扩展桥接只注册文档和范围格式化。文档格式化调度由 `contrib/format/browser/format.ts` 承担：先查询整文提供者，再查询未被同一扩展覆盖的范围提供者；扩展身份比较忽略大小写。提供者失败通过公共错误处理器报告后继续查询。控制器拥有取消与编辑提交，worker 负责最小化文本编辑并保留换行符；资源级 worker-service API 仍待对齐。其他功能的 registry 仍从贡献导入部分类型，这是待修正的依赖，不是允许其他能力沿用的模式。
