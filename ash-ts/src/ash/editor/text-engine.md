...
选区格式化的查询和交叠处理由 `FormatController` 管理：提供者把不同选区扩展到相同内容时，合并请求范围重新查询，旧结果及其 EOL 指令一起丢弃。查询阶段不写入模型；结果确定后才经 worker 最小化，由 `FormattingEdit` 一次提交。查询期间编辑器状态改变或请求取消，不产生格式化历史记录。
