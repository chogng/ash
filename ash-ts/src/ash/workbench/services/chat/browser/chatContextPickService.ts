...
import type { ChatContextAttachment, ChatContextPick, ChatContextPicker, IChatContextPickService, ResolvedChatAttachment } from '../common/chatContextService.js';

interface PickerItem extends IQuickPickItem {
	readonly picker: ChatContextPicker<ResolvedChatAttachment>;
}

interface ContextItem extends IQuickPickItem {
	readonly attachment: ChatContextAttachment<ResolvedChatAttachment>;
}

export class ChatContextPickService implements IChatContextPickService {
	private readonly pickers = new Map<string, ChatContextPicker<ResolvedChatAttachment>>();
	...
	async pickContext(quickInputService: IQuickInputService): Promise<ChatContextAttachment<ResolvedChatAttachment> | undefined> {
		const enabled: ChatContextPicker<ResolvedChatAttachment>[] = [];
...
async function selectContext(quickInputService: IQuickInputService, provider: ChatContextPicker<ResolvedChatAttachment>): Promise<ChatContextAttachment<ResolvedChatAttachment> | undefined> {
	...
	let generation = 0;
	let settled = false;
	return new Promise<ChatContextAttachment<ResolvedChatAttachment> | undefined>(resolve => {
		const finish = (attachment: ChatContextAttachment<ResolvedChatAttachment> | undefined): void => {
			if (settled) return;
			settled = true;
			resolve(attachment);
			resources.dispose();
...
function toContextItem(pick: ChatContextPick<ResolvedChatAttachment>): ContextItem {
	return {
		label: pick.label,
