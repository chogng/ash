...
| `provider/apiKey/set` / `provider/list` | model provider credential | 受信任的本地客户端提交新 key，由 App Server 写入 profile SecretStore；列表只返回配置状态，不返回 key。CLI/TUI 使用同一协议，Desktop 可通过生成的协议客户端调用，当前尚无专门的 key 设置界面。 |
