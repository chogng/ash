...
执行设置由接受请求时写入的 `Turn` 保存：指令、模型、Advisor、工具范围、工具模式和审批模式都随 Turn 冻结。重启后准备模型请求仍读取这个 Turn；压缩摘要只记录被覆盖的上下文，不重复保存执行设置。派生子 Agent 时，协调器从父 Turn 生成不可变种子。旧记录若缺少当时冻结的指令，会明确拒绝恢复执行，不能用当前用户配置猜测历史设置。
