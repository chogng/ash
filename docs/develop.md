...
本文正在设计一个系统。`ASH.md` 已由 Agent 运行时读取；`intent.md`、`spec.md` 和 `plan.md` 是该系统未来产生的开发产物，不用于把当前系统设计拆成四份自我描述的文档。
...
讨论板只保存协作信息。已接受产物、阶段转换、失效判断和最终验收继续由开发流程记录；帖子中的“通过”或“完成”不能推动状态机。该接入方式属于设计，`/develop` 命令和阶段协调内核仍未实现。
