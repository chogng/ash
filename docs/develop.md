...
| 命令 | 当前行为 |
| --- | --- |
| `/develop <任务>` | 创建工作并启动 Intent；同一根 Thread 同时只允许一个活动工作 |
| `/develop` | 有活动工作时恢复；首次使用时以最近用户任务及冻结对话前缀作为输入 |
| `/develop status` | 读取当前阶段、候选版本、问题和证据；不调用模型 |
| `/develop accept <版本号>` | 接受当前成功候选并启动下一阶段；接受 Acceptance 后收口 |
| `/develop resume [用户决定]` | 保留已有 Agent 身份恢复运行；失败或待决定时创建新的阶段尝试 |
| `/develop revise <阶段> <原因>` | 阶段名为 `intent/spec/plan/implementation/acceptance`；取消当前执行，保留历史并使该阶段及下游失效 |
| `/develop cancel` | 取消当前阶段及其后代，保留已接受产物与历史 |

- 阶段顺序固定为 Intent → Spec → Plan → Implementation → Acceptance，每阶段使用独立角色和 Thread。阶段 Agent 返回带 `outcome/content/evidence` 的 JSON 候选，不能调用接受或推进命令。
...
`ASH.md` 对应项目级 Agent 开发上下文，由当前 Agent 运行时作为 always-on Instruction 读取，包含构建与验证入口、长期约束、架构入口以及 Agent 容易重复犯下的错误。它不是某一次开发工作的意图，也不复制全部系统文档。
...
`ASH.md` 位于项目根并由 Agent 运行时读取。下面的 `.ash/work/` 仍只表达计划中的职责关系，不是已经接受的最终路径：

```text
ASH.md                    # 已接入的项目级 Agent 开发上下文
.ash/
└── work/
    └── <work-id>/
...
| D2 单 Agent 纵向切片 | 部分具备 | 命令、候选版本、显式接受、恢复、取消和失效已接入 | Git 基线、封存与真实任务评测仍需完成 |
| D3 Intent 私有 Agent | 部分具备 | 三个角色已打包；准确调用方与工具门禁已执行 | 证据质量与停止条件仍需真实模型评测 |
| D4 Team 执行 | 部分具备 | `/team` 及实施阶段内的团队委托已接入 | 文件范围执行限制和工作图验证仍需完成 |
| D5 `.ash/work` 固定产物 | 待开始 | 根据真实使用结果确定工作产物的文件契约 | 发现、版本、权限、迁移和多工作并存通过端到端验证 |

首个实现必须是完整但窄的单 Agent 纵向切片。不能先只建立多个角色定义或界面，再让流程状态、版本失效和恢复以后补齐。

## 15. 当前未决问题

- 当前 `/develop` 优先恢复活动工作；多工作选择入口仍需设计；
- 同一对话存在多个独立问题时，用户如何选择命令锚点和目标范围；
- 哪些清楚的小型任务可以自动跳过单独的 Spec 或 Plan 产物，同时仍保留验收依据；
- Intent、Spec 和 Plan 的接受权限如何映射个人项目与多人项目；
- 项目级 `ASH.md`、共享 `AGENTS.md` 与已有 Instructions 如何避免重复规则；
- 当前工作产物使用持久化领域对象；普通文件的导出与绑定仍需设计；
