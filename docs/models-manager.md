...
当前 TUI 的 `model/list(view=discovered)` 按供应商分组；同一供应商内保留其订阅目录的展示顺序。
...
字符串来推断发布时间。新增订阅来源若提供明确的展示优先级，应在来源内按该字段稳定排序；
否则保留来源列表顺序，并在对应适配器说明其可靠性。

下面的跨供应商通用 picker 排序仍是后续设计，当前 `discovered` 列表未按这些字段重排：
