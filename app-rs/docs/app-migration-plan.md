...
> 状态：当前迁移计划。本文拥有 `app` 从 `ash-rs/native` 迁移到仓库根 `app-rs/` 的阶段、边界和验收条件；
...
本计划最初以仓库根的 `app/` 完成迁移；当前目录名为 `app-rs/`，下文目标路径均使用现名。

## 快速理解

`ash-rs` 作为 `app-ts` 和 `app` 共用的 Rust backend；`app-rs/` 作为 `app` 的独立产品宿主；
`app-ts/` 继续作为 Electron 宿主。当前 `ash-rs/native` 把产品宿主、Native UI framework 和 GPU
...
app ────→ ash-rs backend/domain
             └─→ app UI workspace
ash-rs ───────→ no app-rs/desktop product host
```
...
- [x] 在仓库根建立 `app-rs/` 产品目录，并以 `app` 作为发布 binary；
- [x] 将当前 `ash-rs/native` 的产品宿主、测试和 README 整体迁移到 `app-rs/`，保留现有模块边界和行为；
...
`app-rs/` 的直接子目录。现在由仓库根 Cargo workspace 统一解析所有 crate，app 仍作为独立 package 和发布
边界存在；Bazel 使用同一个 root graph。

### 阶段二：分离 Native UI workspace（核心迁移已完成）

- [x] 将 Native UI crates 迁入 `app-rs/`，再把 icon/UI/runtime/wgpu/winit 职责收敛为单一 public `zui` crate 的同名能力目录；
- [x] 将 `ash-editor` 迁入 App；当时保留在共享目录的编辑核心，已按[前端 crate 迁移计划](crate-migration-plan.md)继续迁入 `app-rs/editor-core`；`ash-syntax` 保留共享算法；
...
当前根 `Cargo.toml` 是唯一 workspace，`app-rs/Cargo.toml` 是 `app` package。`app-rs/BUILD.bazel` 已
提供 `//app-rs:app`、`//app-rs:app_sources`、`//app-rs:app_release_inputs`、
`//app-rs:app_package_contract` 和 `//app-rs:app_ci`；所有 app/shared crates 通过单一 `@crates`
hub 消费 rules_rs 生成的 package deps。`bazel build //app-rs:app` 已在当前 macOS toolchain 通过；
此前由 apple-cf Swift bridge 引起的限制已随 zui 的字体目录实现迁移而消除。

### 阶段三：收敛应用组合根

- [x] 删除 `app-rs/src`；`app-rs/main.rs` 只启动 `ash-workbench`，窗口/平台适配、产品状态、command 执行和具体 Part/Overlay 组合统一由 Workbench 管理；
...
- [x] 为 `app` 建立发布输入和 boundary CI graph；`//app-rs:app_ci` 验证 workspace ownership，
      `//app-rs:app_release_inputs` 固定 package/signing contract；Cargo workspace 仍是唯一 canonical
      Rust build graph；
- [x] 建立 `app-rs/` 与 `ash-rs` shared crates 的 metadata-derived hermetic Bazel Rust graph；
      单一 root workspace 已消除跨 workspace path dependency bridge；
- [x] 在 CI 入口中通过完整 `//app-rs:app` build 验证平台 toolchain；本机已通过同一 target，
      `//app-rs:app_ci` 继续验证 boundary/package/signing contract。
- [x] 提供平台签名与验证入口；`build/app_rs/build.py` 负责 Build → Stage，
      `build/app_rs/signing.py` 负责 Sign → Verify；平台任务绑定签名工具和凭据，只发布
      `verified` artifact；
- [x] `app-ts` 构建不拉入 Native UI/GPU 依赖；desktop 侧只保留自身的 platform adapter，边界检查未发现
...
- [x] 将 Session、Workspace、Editor 和 Remote 的可复用状态/视图按 owner 分批抽到
      `app-rs/` 下的产品 crate；组合根只保留宿主快照、effect 和平台事件接线；
...
check/test 验证；Bazel graph 的完整入口是 `bazel build //app-rs:app`，发布 contract 入口是
`bazel test //app-rs:app_ci --test_env=PATH`，package staging 使用 `just app-package`，
签名和验证使用 `build/app_rs/signing.py`，发布细节见
[`app-release-graph.md`](app-release-graph.md)。

并满足以下结构检查：

- `rg 'ash-rs/native|ash-native'` 只剩迁移历史、兼容说明或明确的旧版本记录；
- 根 `Cargo.toml` 是唯一 Cargo workspace；`app-rs/Cargo.toml` 不声明 nested workspace；
