...
- Main-branch push CI compares the protocol package against the previous revision on the same runner and toolchain, using three samples. A slowdown exceeding both 25% and two seconds fails the gate. Inspect the uploaded before/after reports and repeat noisy runs; do not increase the threshold solely to pass a change.
