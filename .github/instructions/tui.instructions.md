...
2. 页签、搜索框和列表交给共用组件处理。打开列表时聚焦默认选中项；没有明确预选项时选首个可见的非分组标题项目。`Tab` / `Shift+Tab`、页签栏左右键及点击页签切页后，聚焦目标页首个可见项目；当前筛选下没有项目时，焦点留在页签栏。页签和搜索框不算进入页面时的默认命中项。页签栏的上下键按页签栏、独立动作（若有）、搜索框（若有）、列表的视觉顺序移动焦点；列表内的上下键先逐项移动，到边界才离开列表。
