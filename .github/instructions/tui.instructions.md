...
验收时按功能 owner 测试按键、焦点、搜索、返回和业务结果；fullscreen 与 inline 分别验证面板呈现与关闭后的焦点。涉及鼠标时验证命中与键盘激活走同一动作，尺寸变化取消未完成的点击；涉及异步请求时验证关闭、替换或切换模式后的迟到结果不会重开旧面板。具体方法按下文的 [test-tui](../../.agents/skills/test-tui/SKILL.md) 要求执行。
